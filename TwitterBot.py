...
= CREDITS = 
Based on the initial work made by @gmellini
https://github.com/gmellini/twitter-scraper
...
e["consumer_key"]="4o4XKErd9uaK5TnBBmBpFcXqZ"
e['consumer_secret']="Nm42UXHSoFPqtvwlHKXAVblW47qTDgJkkUDxG9EOLWGhE7DdRt"
e["access_token_key"]="4302974298-Tg1wMjwCEpvOGizjvhc6G38pLv7DQdl1ObgCgV4"
e['access_token_secret']="HtfpdijwIAEQgvPWmyc4nuTQnW91kt1rEeI4DK1F7nluB"
    ...
    for tweet in get_tweets(file_in):
        if tweet_url is not None:
            ...
            for reply in get_replies(tweet):
                ...
                human_date = datetime.strptime(pre_human_date, '%a %b %d %H:%M:%S %Y').replace(tzinfo=timezone('IST')).astimezone(tz=timezone(local_timezone)).strftime('%d/%m/%Y %H:%M:%S')
                count += 1
